...
const bankedBalances: { [shipId: string]: { [year: number]: number } } = {};
...
app.get('/routes/comparison', async (req, res) => {
  try {
    ...
    if (!baseline) return res.status(200).json([]);
...
app.post('/banking/bank', async (req, res) => {
  try {
    ...
    // Bank the surplus
    if (!bankedBalances[shipId]) bankedBalances[shipId] = {};
...
app.post('/pools', async (req, res) => {
  try {
    ...
    // Validate pool rules
    const totalCB = members.reduce((sum: number, m: any) => sum + m.cbBefore, 0);
    if (totalCB < 0) return res.status(400).json({ error: 'Pool sum must be >= 0' });

    // Distribute CB equally among members
    const numMembers = members.length;
    const equalCB = totalCB / numMembers;

    const poolMembers = members.map((m: any) => {
